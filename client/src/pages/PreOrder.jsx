...
const PreOrder = () => {
	...
	useEffect(() => {
		...
		const fetchFurnitureDetails = async () => {
			...
			try {
				...
				// setExpectedDelivery(newFurniture.furnitureType.ECT);
	...
	// For partial payment
	const partialPayment = ((subtotal * quantity + shippingFee) / 2).toFixed(2);
	...
	useEffect(() => {
		if (furnitureData) {
			...
			console.log(expectedDelivery); // Debugging the state update
		}
	}, [furnitureData]);
	...
	const preOrder = async () => {
		if (!selectedColor || !selectedMaterial || !selectedSize) {
			alert("Please select color, material and size before checking out.");
			return;
		}

		if (!selectedPaymentMethod) {
			alert("Please select payment method checking out.");
			return;
		}
		...
		formData.append("expectedDelivery", JSON.stringify(expectedDelivery));
	...
	return (
		<section className="bg-white">
			...
			<div className="flex flex-col lg:flex-row lg:w-full justify-center p-5 gap-5 ">
				{/* Image Section */}
				<div className="flex flex-col rounded-xl p-5 border-2 border-green-600 shadow-lg shadow-gray-300 w-full md:w-1/2 lg:w-1/4 h-1/2">
					{/* Back Button */}
					<button
						onClick={() => navigate(-1)}
					...
					{furnitureData?.images?.length > 0 && (
						<div className="flex flex-col items-center mt-2 h-full">
							<img
								...
								className="w-full h-1/4 object-contain rounded"
							/>
							...
							<div className="flex items-center justify-center mt-2 space-x-4">
								<button
									onClick={() =>
										setCurrentImageIndex((i) =>
											i === 0 ? furnitureData.images.length - 1 : i - 1
										)
									}
								...
								<div className="flex space-x-2 overflow-x-auto p-2">
									{furnitureData.images.map((image, i) => (
										<img
											key={i}
								...
								<button
									onClick={() =>
										setCurrentImageIndex((i) =>
											i === furnitureData.images.length - 1 ? 0 : i + 1
										)
									}
									className="text-gray-500 hover:text-green-600"
								>
								...
								<div className="flex text-lg flex-col-2 gap-2 mb-2">
									{furnitureData.sizes?.map((size) => (
										<div
											...
											className={`relative border px-2 py-1 rounded-md border-green-600 hover:bg-green-600 hover:text-white cursor-pointer transition ${
								...
								{paymentOption === "Partial Payment" && (
									<>
										<label className="italic text-xs text-justify">
											...
											installments with a minimum of at least 50% down payment.
											The remaining balance can be split over 3 months. A 3%
											interest fee will be applied if the customer will be late
											in payments.
										</label>
						...
						{selectedDeliveryMode === "Delivery" && (
							<div className="mt-4 p-4 bg-slate-100 rounded-md">
								...
								<p className="text-base text-black mt-2">
									...
									{expectedDelivery.startDate instanceof Date
										? expectedDelivery.startDate.toLocaleDateString()
										: "N/A"}{" "}
									-{" "}
									{expectedDelivery.endDate instanceof Date
										? expectedDelivery.endDate.toLocaleDateString()
										: "N/A"}
								</p>
							</div>
						)}
