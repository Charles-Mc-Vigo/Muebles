	...
	return (
		<div>
			<div className="flex flex-wrap justify-between items-center max-w-6xl mx-auto p-5">
				...
				<div className={`w-full lg:w-auto ${isMenuOpen ? 'block' : 'hidden'} lg:block text-right`}>
				...
				<div className={`w-full lg:w-auto ${isMenuOpen ? 'block' : 'hidden'} lg:block mt-4 lg:mt-0 text-right`}>
					<ul className="flex flex-col lg:flex-row space-y-2 lg:space-y-0 lg:space-x-4">
						<Link to="/login"><li className="hover:opacity-60 hover:text-white font-semibold">Log in</li></Link>
						<Link to="/signup"><li className="hover:opacity-60 hover:text-white font-semibold">Sign in</li></Link>
					</ul>
				</div>
			</div>
		</div>
