...
const Header = ({ isLogin }) => {
  ...
  return (
    <>
      {/* Header for Desktop */}
      <div className="flex hidden md:flex justify-center mr-5 items-center p-4 bg-white shadow-md">
        {/* Left - Logo */}
      ...
      {/* Header for Mobile */}
      <header className="grid md:hidden  bg-white shadow-md justify-center space-x-10 items-center p-4">
        {/* Menu Button */}
            ...
            <ul className="flex flex-col items-start p-4">
              <div className="flex items-center justify-between w-full text-sm text-black py-2">
                <div className="flex items-start">
                  {isLogin ? (
                    <Link to="/logout" onClick={toggleMenu} className="mr-2">
                      Logout
                    </Link>
                  ) : (
                    <>
                      <Link to="/login" onClick={toggleMenu} className="mr-2">
                        Login
                      </Link>
                      <span className="mx-1">|</span>
                      <Link to="/signup" onClick={toggleMenu} className="ml-2">
                        Sign Up
                      </Link>
                    </>
                  )}
