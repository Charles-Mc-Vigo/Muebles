            ...
            {["All", "Pending", "Confirmed", "Delivered", "Cancelled"].map((status) => (
              <button
                ...
                className={`px-4 py-2 rounded-lg transition-colors 
                  ${filter === status ? "bg-blue-500 text-white" : "bg-gray-200 text-gray-800 hover:bg-blue-100"}`}
              >
                {status}
              </button>
            ))}
                  ...
                  {order.type === "Pre-Order" ? (
                    <div>
                        ...
                        <div className="bg-white rounded-lg shadow-sm p-4 border border-gray-100">
                          <div className="flex items-center space-x-4">
                            <div className="bg-gray-50 p-2 rounded w-24 h-24 flex items-center justify-center">
                              {order.furniture.images && order.furniture.images.length > 0 && (
                                <img
                            ...
                            <div className="flex-grow">
                              <p className="font-medium text-gray-800">{order.furniture.name}</p>
                              <div className="mt-2 space-y-1">
                                <p className="text-sm text-gray-600">Quantity: {order.quantity}</p>
                                <p className="text-sm text-gray-600">Price: PHP {order.subtotal}</p>
                              </div>
                            </div>
                          </div>
                        </div>
                      ...
                      <div className="space-y-3">
                        ...
                        <p className="text-gray-600">Shipping Fee: PHP {order.shippingFee}</p>
                          ...
                          {order.items.map((item, itemIndex) => (
                            ...
                            >
                              <div className="flex items-center space-x-4">
                                ...
                                <div className="flex-grow">
                                  <p className="font-medium text-gray-800">{item.furniture.name}</p>
                                  <div className="mt-2 space-y-1">
                                    <p className="text-sm text-gray-600">Quantity: {item.quantity}</p>
                                    <p className="text-sm text-gray-600">Price: PHP {item.price}</p>
                                  </div>
                                </div>
                              </div>
                            </div>
