					...
					<div className="bg-white rounded-lg shadow-md p-6">
						...
						<ul className="space-y-2 text-black">
									...
									{order.user.addresses
										...
										.map((defaultAddress, index) => (
											<p key={index} className="tracking-wide mt-1">
												...
												{defaultAddress.barangay}, {defaultAddress.municipality}, {defaultAddress.zipCode}
											</p>
										))}
								</p>
							</li>
							<li>
								...
								<span className={`${
									order.orderStatus === "pending" ? "text-yellow-600" : "text-green-700"
								} font-semibold`}>
									{order.orderStatus}
								</span>
							</li>
					...
					<div className="bg-white rounded-lg shadow-md p-6">
							...
							<table className="min-w-full border-collapse border border-gray-200">
								<thead>
									<tr className="bg-gray-200">
										<th className="py-3 px-4 text-left font-medium text-black border border-gray-300">Item</th>
										<th className="py-3 px-4 text-left font-medium text-black border border-gray-300">Specifications</th>
										<th className="py-3 px-4 text-left font-medium text-black border border-gray-300">Quantity</th>
										<th className="py-3 px-4 text-left font-medium text-black border border-gray-300">Price</th>
										<th className="py-3 px-4 text-left font-medium text-black border border-gray-300">Total</th>
									</tr>
								</thead>
								<tbody>
									{order.type === "Pre-Order" ? (
										<tr className="hover:bg-gray-50">
											<td className="py-3 px-4 text-black border border-gray-300 flex items-center">
												...
												<span className="font-medium">{order.furniture?.name}</span>
											</td>
											...
											<td className="py-3 px-4 text-black border border-gray-300">{order.quantity}</td>
											<td className="py-3 px-4 text-black border border-gray-300">₱{(order.totalAmount / order.quantity).toFixed(2)}</td>
											<td className="py-3 px-4 text-black border border-gray-300">₱{order.totalAmount.toFixed(2)}</td>
										</tr>
									) : (
											...
											<tr key={item._id} className="hover:bg-gray-50">
												<td className="py-3 px-4 text-black border border-gray-300">
													<div className="flex items-center space-x-4">
														...
														<span className="font-medium">{item.furniture?.name}</span>
													</div>
												</td>
												<td className="py-3 px-4 text-black border border-gray-300">
													<div className="space-y-1">
												...
												<td className="py-3 px-4 text-black border border-gray-300">{item.quantity}</td>
												<td className="py-3 px-4 text-black border border-gray-300">₱{item.price.toFixed(2)}</td>
												<td className="py-3 px-4 text-black border border-gray-300">₱{(item.price * item.quantity).toFixed(2)}</td>
											</tr>
										))
									)}
								</tbody>
						...
						<div className="mt-6 space-y-2">
							...
							<div className="flex justify-between font-medium text-xl text-gray-800 pt-2 border-t">
								<span>Total:</span>
								<span>₱{order.totalAmountWithShipping?.toFixed(2) || "0.00"}</span>
							</div>
							{order.paymentOption === "Partial Payment" && (
								<div className="mt-4 pt-4 border-t space-y-2">
									...
									<div className="flex justify-between text-gray-800">
										...
										<span>₱{order.monthlyInstallment?.toFixed(2) || "0.00"}</span>
									</div>
								</div>
							)}
						</div>
